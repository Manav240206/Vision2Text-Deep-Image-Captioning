...
get_ipython().system('unzip "/content/drive/MyDrive/flickr8k.zip" -d "/content/flickr8k"')
...
def lr_scheduler(epoch, lr):
    # Using math.exp ensures the output is a float, not a tensor
    return float(lr * math.exp(-0.1))

lr_schedule = LearningRateScheduler(lr_scheduler)


# 2. Define Output Signature (matching the generator's nested tuple)
output_signature = (
...
# 4. Train
# Note: Lower the batch size if you get an "Out of Memory" (OOM) error
...
for caption in test_captions:
    # Explicitly use \t instead of a literal tab space
    image_id, cap = caption.split('\t')
    if image_id not in test_actual_captions:
        test_actual_captions[image_id] = []

    # .strip() cleanly removes any \n or extra spaces
    test_actual_captions[image_id].append(cap.strip())
...
# 1. Save and download the Model
model_filename = 'image_captioning_model.h5'
caption_model.save(model_filename)
files.download(model_filename)

# 2. Save and download the Tokenizer
...
def generate_caption(image_path):
    try:
        # 1. Extract features using the InceptionV3 model
        features = extract_image_features(inception_v3_model, image_path)
        ...
        print(f"Error: Could not find the image at '{image_path}'. Double check the path!")

# 👇 CHANGE THIS TO YOUR IMAGE'S EXACT LOCATION IN COLAB 👇
my_image_location = '/content/beach.webp'

# Run the model on your image
generate_caption(my_image_location)
